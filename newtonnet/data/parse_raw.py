...
def parse_train_test(settings, device):
    ...
    val_data = dict(np.load(val_path))
    ...
    if train_size != -1:
        n_select = sample_without_replacement(
            train_data['R'].shape[0], 
            train_size, 
            random_state=random_state,
            )
        for key in train_data.keys():
            train_data[key] = train_data[key][n_select]
    if val_size != -1:
        n_select = sample_without_replacement(
            val_data['R'].shape[0], 
            val_size, 
        ...
        for key in val_data.keys():
            val_data[key] = val_data[key][n_select]
    if test_size != -1:
        n_select = sample_without_replacement(
            test_data['R'].shape[0], 
            test_size, 
        ...
        for key in test_data.keys():
            test_data[key] = test_data[key][n_select]

    # extract data stats
    normalizer = (train_data['E'].mean(), train_data['E'].std())

    n_train_data = train_data['R'].shape[0]
    n_val_data = val_data['R'].shape[0]
    ...
    train_gen = extensive_train_loader(data=train_data,
                                       env_provider=env,
    ...
    test_gen = extensive_train_loader(data=test_data,
                                     env_provider=env,
                                     batch_size=val_batch_size,
                                     device=device,
                                     shuffle=False,
    ...
    return train_gen, val_gen, test_gen, train_steps, val_steps, test_steps, normalizer
